...
class QRPay {
    ...
    build() {
        var _a;
        const version = QRPay.genFieldData(constants_1.FieldID.VERSION, this.version);
        const initMethod = QRPay.genFieldData(constants_1.FieldID.INIT_METHOD, this.initMethod);
        const guid = QRPay.genFieldData(constants_1.ProviderFieldID.GUID, this.provider.guid);
        let providerDataContent = '';
        if (this.provider.guid === constants_1.QRProviderGUID.VIETQR) {
            const bankBin = QRPay.genFieldData(constants_1.VietQRConsumerFieldID.BANK_BIN, this.consumer.bankBin);
            const bankNumber = QRPay.genFieldData(constants_1.VietQRConsumerFieldID.BANK_NUMBER, this.consumer.bankNumber);
            providerDataContent = bankBin + bankNumber;
        }
        else if (this.provider.guid === constants_1.QRProviderGUID.VNPAY) {
            providerDataContent = (_a = this.merchant.merchantId) !== null && _a !== void 0 ? _a : '';
        }
        const provider = QRPay.genFieldData(constants_1.ProviderFieldID.DATA, providerDataContent);
        const service = QRPay.genFieldData(constants_1.ProviderFieldID.SERVICE, this.provider.service);
        const providerData = QRPay.genFieldData(this.provider.fieldId, guid + provider + service);
        const category = QRPay.genFieldData(constants_1.FieldID.CATEGORY, this.category);
        const currency = QRPay.genFieldData(constants_1.FieldID.CURRENCY, this.currency);
        const amountStr = QRPay.genFieldData(constants_1.FieldID.AMOUNT, this.amount);
        const tipAndFeeType = QRPay.genFieldData(constants_1.FieldID.TIP_AND_FEE_TYPE, this.tipAndFeeType);
        const tipAndFeeAmount = QRPay.genFieldData(constants_1.FieldID.TIP_AND_FEE_AMOUNT, this.tipAndFeeAmount);
        const tipAndFeePercent = QRPay.genFieldData(constants_1.FieldID.TIP_AND_FEE_PERCENT, this.tipAndFeePercent);
        const nation = QRPay.genFieldData(constants_1.FieldID.NATION, this.nation);
        const acquier = QRPay.genFieldData(constants_1.FieldID.ACQUIER, this.acquier);
        const city = QRPay.genFieldData(constants_1.FieldID.CITY, this.city);
        const zipCode = QRPay.genFieldData(constants_1.FieldID.ZIP_CODE, this.zipCode);
        const buildNumber = QRPay.genFieldData(constants_1.AdditionalDataID.BILL_NUMBER, this.additionalData.billNumber);
        const mobileNumber = QRPay.genFieldData(constants_1.AdditionalDataID.MOBILE_NUMBER, this.additionalData.mobileNumber);
        const storeLabel = QRPay.genFieldData(constants_1.AdditionalDataID.STORE_LABEL, this.additionalData.store);
        const loyaltyNumber = QRPay.genFieldData(constants_1.AdditionalDataID.LOYALTY_NUMBER, this.additionalData.loyaltyNumber);
        const reference = QRPay.genFieldData(constants_1.AdditionalDataID.REFERENCE_LABEL, this.additionalData.reference);
        const customerLabel = QRPay.genFieldData(constants_1.AdditionalDataID.CUSTOMER_LABEL, this.additionalData.customerLabel);
        const terminal = QRPay.genFieldData(constants_1.AdditionalDataID.TERMINAL_LABEL, this.additionalData.terminal);
        const purpose = QRPay.genFieldData(constants_1.AdditionalDataID.PURPOSE_OF_TRANSACTION, this.additionalData.purpose);
        const dataRequest = QRPay.genFieldData(constants_1.AdditionalDataID.ADDITIONAL_CONSUMER_DATA_REQUEST, this.additionalData.dataRequest);
        const additionalDataContent = buildNumber +
            mobileNumber +
            storeLabel +
            loyaltyNumber +
            reference +
            ...
            purpose +
            dataRequest;
        const additionalData = QRPay.genFieldData(constants_1.FieldID.ADDITIONAL_DATA, additionalDataContent);
